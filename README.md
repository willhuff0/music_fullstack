...
The class mainly deals with Dart's threading system. Dart doesn't allow shared memory between threads. To exchange information, messages have to be sent over 'ports', which can be a pain and is why I confine that logic to these classes.
...
This class implements the Worker interface and allows me to provide a list of CustomHandler objects which take a route and a function to be executed when an http request is received. I extend CustomHandlerBase with CustomHandlerAuthRequired which verifies the session token before calling the provided function. Since the function may be executed on some arbitrary thread, CustomThreadData is passed to allow access to thread bound state, such as the Isar database reference.
...
The list of endpoints in music_server.
...
This snippet from songSearchHandler preforms a query on the stored and indexed phonetic codes of each song's name. Using phonetic codes allows that search to still be fuzzy (such as allowing a spelling error in a song title).
...
Sync sessions allow multiple clients signed in as the same user to play music at the same time.
